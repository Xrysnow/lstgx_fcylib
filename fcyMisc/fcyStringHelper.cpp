...
*/
/*
string fcyStringHelper::TimeToStr(double Seconds)
{
...
wstring fcyStringHelper::TimeToWideStr(double Seconds)
{
	uint32_t tIntPart = (int32_t)Seconds;
	uint32_t ms = (uint32_t)((Seconds - tIntPart) * 1000); // 提出毫秒部分
	uint32_t s = tIntPart % 60;    // 提出秒
...
*/
wstring fcyStringHelper::TrimLeft(const wstring &Org)
{ 
	wstring tRet = Org;
...
wstring fcyStringHelper::TrimRight(const wstring &Org)
{
	wstring tRet = Org;
...
wstring fcyStringHelper::Trim(const wstring &Org)
{
	return TrimLeft(TrimRight(Org));
}

string fcyStringHelper::TrimLeft(const string &Org)
{ 
	string tRet = Org;
...
string fcyStringHelper::TrimRight(const string &Org)
{
	string tRet = Org;
...
string fcyStringHelper::Trim(const string &Org)
{
	return TrimLeft(TrimRight(Org));
}
/*
wstring fcyStringHelper::MultiByteToWideChar(const string& Org, uint32_t CodePage)
{
...
string fcyStringHelper::WideCharToMultiByte(const wstring& Org, uint32_t CodePage)
{
...
wstring fcyStringHelper::MultiByteToWideChar_UTF8(const string& Org)
{
	wstring ret;
	try
	{
...
string fcyStringHelper::WideCharToMultiByte_UTF8(const wstring& Org)
{
	string ret;
	try
	{
